...
# Measures line coverage of the library (lib/) while running the whole test
# suite, using only the Ruby standard library's `coverage` extension -- so it
# adds no gem dependency and runs on the 2.6 support floor. Exits non-zero if
...
at_exit do
  ...
  result.each do |path, counts|
    ...
    report << format('  %-18s %3d/%-3d%s', File.basename(path),
                     executable.count(&:positive?), executable.size,
                     uncovered.empty? ? '' : "  uncovered lines: #{uncovered.join(', ')}")
  end
